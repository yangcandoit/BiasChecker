...
class TwitterSearch {
    ...
    async fetchTweetResults(term, maxResults, TAB) {
        ...
        try {
            ...
            await this.page.waitForSelector(TWEETS_SELECTOR, {
                timeout: 5000
            });
        } catch (e) {
                    ...
                    document.querySelectorAll(TWEETS_SELECTOR).forEach(e => {
                        try {
                            if (TAB && TAB.indexOf('user') !== -1) {
                                ...
                                // 'a[class="css-4rbku5 css-18t94o4 css-901oao r-m0bqgq r-1loqt21 r-1q142lx r-1qd0xha r-a023e6 r-16dba41 r-ad9z0x r-bcqeeo r-3s2u2q r-qvutc0"]'
                                // tweet link
    ...
    async doFollow(profile) {
        ...
        const LIKE_BUTTON_SELECTOR = 'div[aria-label="Like"]'

        const RETWEET_BUTTON_SELECTOR = 'div[aria-label="Retweet"]'
        const _RETWEET_BUTTON_SELECTOR = 'div[role="menuitem"]'
        if (profile.substring(0, 4) == "http") {
            ...
            try {
                await this.page.waitFor(RETWEET_BUTTON_SELECTOR, {
                    timeout: 3000
                });
                await this.page.click(RETWEET_BUTTON_SELECTOR);
                await this.page.waitFor(_RETWEET_BUTTON_SELECTOR, {
                    timeout: 3000
                });
                await this.page.click(_RETWEET_BUTTON_SELECTOR);
            } catch (e) {
                console.log('already followed')
            }
