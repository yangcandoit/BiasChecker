...
class TwitterSearch {
    ...
    async doLogin() {
        ...
        const PASSWORD_INPUT_SELECTOR = 'input[type="password"]';0
    ...
    async doSearch(term, maxResults) {
        ...
        this.results = await this.fetchTweetResults(term, maxResults);
        return this.results;
    }
    ...
    async doQuery(term) {
        // // MAKE QUERY
        // const EXPLORE_ANCHOR = 'a[href="/explore"]';
        const SEARCH_INPUT_SELECTOR = 'input[id="search"]';
    ...
    async fetchVideoResults(term, maxResults) {
        

        const videoSelector='a[id="video-title"]'
    ...
    async doFollow(profile) {
        const FOLLOW_BUTTON_SELECTOR = "div[id='subscribe-button'][class='style-scope ytd-c4-tabbed-header-renderer']";
        await this.page.goto(`https://www.youtube.com/channel/${profile}`);
        try{
